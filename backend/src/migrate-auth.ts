 ...
 *
 * Ensures all required BetterAuth tables (+ Jazz plugin columns) exist. The Jazz
 * hierarchy backend does NOT create these, so we own the schema. This runs on
 * server boot from index.ts (the systemd unit launches dist/index.js directly
 * and is regenerated from a deploy template, so a migrate step in the unit would
 * not survive a deploy). Also runnable as a CLI: `node dist/migrate-auth.js`.
 */

import Database from 'better-sqlite3';

/**
 * Create the BetterAuth core tables and backfill Jazz plugin columns if missing.
...
export function ensureAuthTables(db: Database.Database): void {
  ...
  if (missingTables.length > 0) {
    ...
    // Schema matches what BetterAuth creates + the jazz-tools plugin columns.
    db.exec(`
