 ...
 *
 * Mode transitions:
 * - NORMAL → ADDING (open add form)
 * - NORMAL → EDITING (double-tap item)
 * - NORMAL/ADDING → DRAGGING (long-press drag)
 * - EDITING → NORMAL/ADDING (save/cancel edit)
 * - DRAGGING → NORMAL/ADDING (drop item)
 * - ADDING → NORMAL (close add form)
 */
export function useSessionInteractionMode() {
  ...
  const enterEditMode = useCallback((itemId: string) => {
    console.log('[InteractionMode] Entering EDIT mode for:', itemId);
    setInteractionMode({ mode: 'editing', itemId });
  }, []);

  const enterDragMode = useCallback((itemId: string) => {
    console.log('[InteractionMode] Entering DRAG mode for:', itemId);
