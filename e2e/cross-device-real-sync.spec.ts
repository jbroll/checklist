...
test.describe('Cross-Device Real Sync - Anonymous Mode', () => {
  test('should create folder on Device A and persist locally', async ({ browser }) => {
    ...
    try {
      ...
      await expect(deviceA.getByRole('heading', { level: 1 })).toBeVisible({
        timeout: 10000,
      });
  ...
  test('should create folder on anonymous Device B before "login"', async ({ browser }) => {
    ...
    try {
      ...
      // Wait for app to load
      await expect(deviceB.getByRole('heading', { level: 1 })).toBeVisible({
        timeout: 10000,
      });
  ...
  test('should show different account IDs for Device A and Device B (anonymous mode)', async ({
    ...
    try {
      ...
      await Promise.all([
        expect(deviceA.getByRole('heading', { level: 1 })).toBeVisible({
          timeout: 10000,
        }),
        expect(deviceB.getByRole('heading', { level: 1 })).toBeVisible({
          timeout: 10000,
        }),
      ]);
